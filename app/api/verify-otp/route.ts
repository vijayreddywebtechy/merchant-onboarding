...
export async function POST(request: NextRequest) {
  try {
    const { mobile, otpValue, otpQName } = await request.json();
    const { code = "27", number } = mobile;

    if (!number || !otpValue || !otpQName) {
      return NextResponse.json(
        { error: "Mobile number, OTP value, and OTP QName are required" },
        { status: 400 }
      );
    }
      ...
      <soapenv:Body>
        <ver:VERSIONROperation>
            ...
            <ver:otp_data>
                ...
                <ver:otp_otp>${otpValue}</ver:otp_otp>
                ...
                <ver:otp_qname>${otpQName}</ver:otp_qname>
