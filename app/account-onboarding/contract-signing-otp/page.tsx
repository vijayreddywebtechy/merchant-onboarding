...
const ContractSigningOTP: React.FC = () => {
  ...
  useEffect(() => {
    // Check if contract was marked for signing
    const contractPendingSign = localStorage.getItem("contractPendingSign");
    if (contractPendingSign !== "true") {
      // Redirect back if not coming from contract signing flow
  ...
  const verifyOtp = async () => {
    ...
    try {
      ...
      if (responseCode === "0000") {
        // Store contract signing verification success
        const verificationData = {
          verified: true,
          timestamp: Date.now(),
          purpose: "contract_signing"
        };
        localStorage.setItem("contract_otp_verification", JSON.stringify(verificationData));
        ...
        setTimeout(() => {
          router.push("/account-onboarding/application-complete");
        }, 1500);
      } else {
        setOtpMessage(getOtpMessage(responseCode || ""));
      }
  ...
  return (
    <div className="page-container py-8 md:py-12">
      <div className="bg-background px-4 py-8 sm:p-10 md:p-12 rounded-xl">
        <div className="mb-8">
          <span className="block text-sm text-gray-600 mb-4">Sign Contract</span>
          <h1 className="text-xl md:text-2xl lg:text-3xl font-medium text-gray-700 mb-6">OTP Verification</h1>
          <p className="text-gray-700 mb-6">
            To sign your contract, please enter the One-Time PIN (OTP) sent to your cellphone number. Never share your OTP with anyone.
          </p>
...
export default ContractSigningOTP;
